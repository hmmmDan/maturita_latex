...
Game managerů se v herních projektech vyskytuje hned několik a každý má na starosti odlišnou funkční část videohry: Game Manager (stará se o přepínání mezi scénami, správné zmrazení času, jednoduché uživatelské rozhraní…), Audio Manager (zajišťuje správný chod veškeré logiky zvuku \newline v prostředí hry nebo v UI), Cursor Manager (upravuje vlastnosti kurzoru počítače pro účely videohry) a další managery, které nejsou zas tak časté, ale mohou se hodit pro konkrétní případy (Tutorial Manager, Collectible Manager, DontDestroyOnLoad, Death Manager).
...
\section{Umírání}
Naši hru jsme navrhli tak, že každý level má dvě části: \emph{The Chase} a \emph{The Exploration}. V \emph{The Chase} je hráč naháněn masivní bowlingovou koulí, která, pokud hráče dožene a přejede ho, tím ho i zabije a hráč bude muset začít znovu. Toto je způsob smrti \emph{hard death}, kdy se po zabití hráče znovu načte celý level, všechny parametry se restartují a hráč si tak musí \emph{The Chase} zapamatovat celou nazpaměť včetně všech překážek.

\emph{The Exploration} využívá více milosrdnou \emph{soft death}, kdy, pokud hráč spadne nebo se mu nějak jinak povede zemřít, místo toho, aby se scéna načetla od začátku, tak je hráč jen přemístěn \newline k poslednímu aktivovanému checkpointu. Můžeme říct, že naše videohra tak používá hybridní systém smrti.

\subsection{Checkpoint System}
Do naší videohry je dobrý nápad zakomponovat \emph{Checkpoint System}, který zajistí, že pokud hráč bude umírat v pozdějších fázích daného levelu, nemusí celým levelem procházet znova, ale hra ho transportuje k nejbližšímu checkpointu, který si uložil.
...
Pro zpestření herního zážitku naší hry jsem přišel s konceptem \emph{Emotion System}. Hráč bude muset v části \emph{The Exploration} sbírat Conversation~Fragments (části konverzace), které obsahují atributy emocí, a na základě kontextu celé konverzace musí hráč správně určit, jestli má daný \newline Conversation~Fragment pozitivní, negativní nebo neutrální emoci. Podle toho pak bude potřeba sesbírané Conversation~Fragments správně umístit na „The Altar“ (oltář), který je schopen aktivovat portál do dalšího levelu v případě správné kombinace.

Díky tomuto konceptu naše hra obsahuje i mírný puzzle-solving a integraci příběhu, které posílí atmosféru surrealismu, kterého chceme v naší videohře docílit. Je totiž důležité, aby videohra \newline neobsahovala pouze akci, ale i části, kde si hráč může oddechnout a připravit se na další akci. Stejně jako v hudbě, kde je složit komplikované mít majestátní, hlasité momenty, pokud správně nepracujete i s tichem. Všechno je to o kontrastu.
...
Dosáhnout těchto efektů můžeme různými způsoby: přidání prvků světla na určitý GameObject \cite{unity-point-light}, experimentování s Unity Particle System \cite{unity-particle-system} nebo manipulování určitývh prvků Post-processing.
...
\begin{itemize}
    \item vzdušné, retro tóny inspirované interpretem Joe Hisaishim (populární japonský skladatel pro legendární filmy od Studio Ghibli), zejména skladbou „The Path of the Wind – Instrumental“
    ...
    \item vinylový efekt
    \item Další inspiraci jsem si vzal například ze skladeb „It’s Just a Burning Memory“ (The Caretaker), „Moog City 2“ (C418) a „Resonance“ (Home).
\end{itemize}
